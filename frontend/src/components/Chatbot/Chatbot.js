...
function Chatbot() {
  const [messages, setMessages] = useState([
    { sender: 'bot', text: 'Hi! I am HireAI, your autonomous HR assistant. How can I help you with recruitment today? 😊' },
    { sender: 'bot', text: 'Feel free to ask me about job applications, resume parsing, candidate ranking, and more! 😊' },
  ]);
  ...
  const sendMessage = async () => {
    if (inputText.trim() && sessionId) {
      ...
      try {
        ...
        const response = await axios.post(`http://localhost:8000/chat/`, { question: inputText });
        console.log('Response from server:', response.data);
        const botMessage = { sender: 'bot', text: response.data.response || 'Something went wrong!' };
  ...
  return (
    <div className="chatbot-container">
      <div className={`chat-icon ${chatVisible ? 'hidden' : ''}`} onClick={toggleChat}>
      ...
      {chatVisible && (
        <div className="chatbox">
          <div className="chatbox-header">
            <div className="chatbox-title">RecruitAI 🤖</div>
